...
class BaseEditor:
    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            ...
            elif 'velvet' in self.model_name.lower():
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map=device_map)
                self.tok = AutoTokenizer.from_pretrained(self.model_name)
                self.tok.pad_token_id = self.tok.eos_token_id 
            ...
            if self.tok is not None and (isinstance(self.tok, GPT2Tokenizer) or isinstance(self.tok, GPT2TokenizerFast) or isinstance(self.tok, LlamaTokenizer)) and (hparams.alg_name not in ['ROME', 'MEMIT', 'memoedit', 'R-ROME', 'regularizedMEMIT', 'lambdamemit']):
                LOG.info('AutoRegressive Model detected, set the padding side of Tokenizer to left...')
                self.tok.padding_side = 'left'
            if self.tok is not None and ('mistral' in self.model_name.lower() or 'llama' in self.model_name.lower() or 'qwen' in self.model_name.lower()) and (hparams.alg_name in ['ROME', 'MEMIT', 'memoedit', 'R-ROME', 'regularizedMEMIT', 'lambdamemit']):
        ...
        if hparams.model_parallel:
            if ":" in self.model.device:
                hparams.device = str(self.model.device).split(":")[1]
            else:
                hparams.device = str(self.model.device)
        if not hparams.model_parallel and hasattr(hparams, 'device'):
            self.model.to(f'{hparams.device}')
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                if self.alg_name == 'KN' or (self.alg_name == 'GRACE' and keep_original_weight):
                    ...
                    self.model = edited_model
                ######new
                elif (self.alg_name == 'ROME' or self.alg_name == 'R-ROME') and not keep_original_weight:
                    print(f"{self.alg_name} in sequential edit")
                    self.model = edited_model
                else:
                    with torch.no_grad():
