...
def get_reprs_at_word_tokens(
    ...
    return get_reprs_at_idxs(
        ...
        module_template,
        track,
        track_subupdates=track_subupdates
    )
...
def get_reprs_at_idxs(
    ...
    """
    #print('called get_reprs_at_idxs')
    ...
    def _process(cur_repr, batch_idxs, key):
        ...
        if key == 'sub':
            #print('sub')
            m, V = cur_repr
            #print("m.shape, V.shape", m.shape, V.shape)
            m = m[torch.arange(m.shape[0]), [[x[0] for x in batch_idxs]]].unsqueeze(dim=-2)
            #print("m.shape", m.shape)
            cur_repr = m * V
            #print("cur_repr.shape", cur_repr.shape)
            to_return[key] = cur_repr
        else:
            cur_repr = cur_repr[0] if type(cur_repr) is tuple else cur_repr
            #print(key, cur_repr.shape, batch_idxs)
            if cur_repr.shape[0]!=len(batch_idxs):
                cur_repr=cur_repr.transpose(0,1)
            for i, idx_list in enumerate(batch_idxs):
                to_return[key].append(cur_repr[i][idx_list].mean(0).unsqueeze(dim=0))
    ...
    for batch_contexts, batch_idxs in _batch(n=128):
        ...
        if tsub:
            _process(tr.sub, batch_idxs, "sub")

            
    to_return = {k: v for k, v in to_return.items() if len(v)}
    to_return = {k: torch.concat(v, 0) if k !='sub' else v for k,v in to_return.items()}
    #print(to_return)
    
    if not both:
        if tin:
            if 'sub' in to_return:
                return to_return['in'], to_return['sub']
            else:
                return to_return['in']
        else:
            if 'sub' in to_return:
                return to_return['out'], to_return['sub']
            else:
                return to_return['out']
    else:
        if 'sub' in to_return:
            return to_return["in"], to_return["out"], to_return["sub"]
        else:
