...
class SafetyEditor:
    ...
    def _locate_toxic_layer(self, model, tokenizer, requests, **kwargs):
        ...
        toxic_layer = []
        input = tokenizer([value for pair in requests for value in [pair["target_new"], pair["ground_truth"]]], return_tensors="pt", padding=True, truncation=True).to(f"{self.hparams.device}") 
        with torch.no_grad():
            outputs = model(**input)
