...
def compute_z(
    ...
    #for it in range(hparams.v_num_grad_steps):
        ...
        if loss < 5e-2:
            break
        if loss < 5e-1 and prob > 0.98:
            break

        ### safety exit
        if it > 2000:
            print("Breaking, max iter")
            break
        ...
        if it_no_gain >= max_no_gain_iter: ## the loss is not moving
            print("Break, not optimizing for too maby iterations")
            if prob < 0.10: # it has been too difficult to optimize
                print(f"Rollbacking changes --> {prob} too low")
                to_rollback = True
            break
        
        if prec_prob is not None and prob is not None and prec_prob > prob and prec_prob > 0.97: 
