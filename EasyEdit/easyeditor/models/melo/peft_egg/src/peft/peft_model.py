...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def load_adapter(self, model_id, adapter_name, is_trainable=False, **kwargs):
        ...
        if use_safetensors:
            adapters_weights = safe_load_file(filename, device="cuda" if torch.cuda.is_available() else "cpu" if torch.cuda.is_available() else "cpu")
        else:
            adapters_weights = torch.load(
                filename, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu" if torch.cuda.is_available() else "cpu")
            )
