    ...
    **kwargs
    ...
    with torch.no_grad():
        for w_name, upd_matrix in deltas.items(): #w_name, adj_k, resid
            upd_matrix = upd_matrix.to("cuda" if torch.cuda.is_available() else "cpu")
        ...
        for rewrite_module_name in rewrite_module_names:
            ...
            if (
                ...
                try:
                    ...
                    z_list[rewrite_module_name].append(torch.from_numpy(data["v_star"]).to("cuda" if torch.cuda.is_available() else "cpu"))
                    data_loaded = True
